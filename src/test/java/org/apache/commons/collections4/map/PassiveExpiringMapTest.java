...
public class PassiveExpiringMapTest<K, V> extends AbstractMapTest<K, V> {
	...
	public static ExpirationPolicy<Integer, String> mockExpirationPolicy1() {
		long mockFieldVariableSerialVersionUID = 1L;
		ExpirationPolicy<Integer, String> mockInstance = mock(ExpirationPolicy.class);
		when(mockInstance.expirationTime(any(), any())).thenAnswer((stubInvo) -> {
			Integer key = stubInvo.getArgument(0);
			if (key == null) {
				return (long) 0;
			}
			if (key.intValue() % 2 == 0) {
...
//    }
	...
	private Map<Integer, String> makeDecoratedTestMap() {
		...
		return new PassiveExpiringMap<>(PassiveExpiringMapTest.mockExpirationPolicy1(), m);
	}
	...
	private Map<Integer, String> makeTestMap() {
		final Map<Integer, String> m = new PassiveExpiringMap<>(PassiveExpiringMapTest.mockExpirationPolicy1());
		m.put(Integer.valueOf(1), "one");
		m.put(Integer.valueOf(2), "two");
		m.put(Integer.valueOf(3), "three");
		m.put(Integer.valueOf(4), "four");
	...
	public void testExpiration() {
		...
		validateExpiration(
				new PassiveExpiringMap<>(
						new PassiveExpiringMap.ConstantTimeToLiveExpirationPolicy<String, String>(1, TimeUnit.SECONDS)),
				1000);
	}
